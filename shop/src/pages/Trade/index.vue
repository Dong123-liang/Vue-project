...
export default {
  ...
  // this.$store.dispatch('getAddressInfo') 获取用户地址接口目前为空，无法获取到数据
  mounted () {
  ...
  methods: {
    async submitOrder () {
      ...
      let data = {
        "consignee": "轨迹",
        "consigneeTel": "15129900990",
        "deliveryAddress": "北京市昌平区2",
        "paymentWay": "ONLINE",
        "orderComment":this.keyword,
        "orderDetailList":this.detailArrayList

      }
      let result =  await this.$http.reqsubmitOrder(tradeNo,data)
      if(result.code ==200){
       this.orderId = result.data // 获取到订单号
