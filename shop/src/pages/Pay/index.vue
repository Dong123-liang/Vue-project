...
export default {
  ...
  methods: {
    async getOrderId () {
      let result = await this.$http.reqPayment(this.orderId)
      // console.log(result)
      if (result.code == 200) {
   ...
   async open(){
        ...
        this.$alert(`<img src=${url} />`, '请使用微信支付',{
          ...
          beforeClose:(action, instance, done) =>{//！！！此处的函数一定要写为箭头函数，this指向向的当前pay组件的实例，若为普通函数则this指向的是beforeClose封装好的组件实例
  ...
  //done 用于关闭 MessageBox 实例
            ...
            if(action == 'cancel'&& this.code!=200){
              alert('请联系管理员轨迹,邮箱:1906232776@qq.com')
    ...
    //查询支付结果,开启定时器每隔一段时间询问支付结果
     ...
     this.timer  = setInterval(async () => {
        //发请求获取支付结果
        let result = await this.$http.reqQueryPayStatus(this.orderId)
        console.log(result)
