...
模拟数据JSON要求：没有空格，最好使用格式化插件进行格式化。
...
本项目中轮播图引用swiper包(5版本以上)
npm iswiper@6/7/8
此项目使用时swiper6.8.4，在项目中引包创建实例时，需要注意为 
import Swiper from “swiper/swiper-bundle”，相当于时项目node_modules下的swiper/swiper-bundle.js,老版本只需要找到/swiper就可，更新版本目前没有亲测
...
watch:监听属性，watch可以检测到属性值的变化，当属性值发生变化的时候，handler函数就会执行，可以出发一次。(此时确保服务器开始响应数据)
